...
class CVBuilder:
    ...
    def _draw_footer(self):
        ...
        text = "CV Forge · Generated with Python + ReportLab"
    ...
    def _draw_header(self, y_start, center_start, center_end):
        ...
        c.setFont(S.FUENTES["bold"], S.TAMAÑOS["nombre"])
        c.setFillColor(self.palette["titulo"])
        nw = c.stringWidth(nombre, S.FUENTES["bold"], S.TAMAÑOS["nombre"])
        c.drawString(center_x - nw / 2, y_start, nombre)

        y = y_start - 22
        c.setFont(S.FUENTES["semibold"], S.TAMAÑOS["seccion"])
        c.setFillColor(self.palette["texto_claro"])
        ow = c.stringWidth(objetivo, S.FUENTES["semibold"], S.TAMAÑOS["seccion"])
    ...
    def _draw_section_title(self, y, title, x_start, x_end):
        self.c.setFont(S.FUENTES["bold"], S.TAMAÑOS["seccion"])
    ...
    def _draw_wrapped(self, y, text, x_start, max_width, font=None, size=None, color=None, bullet=False):
        font = font or S.FUENTES["regular"]
        size = size or S.TAMAÑOS["cuerpo"]
        ...
        inter = S.INTERLINEA["cuerpo"] if size >= S.TAMAÑOS["cuerpo"] else S.INTERLINEA["small"]
        for line in lines:
    ...
    def _draw_experience(self, y, exp, x_start, x_end):
        ...
        c.setFont(S.FUENTES["bold"], S.TAMAÑOS["cuerpo"])
        c.setFillColor(self.palette["titulo"])
        cargo_w = c.stringWidth(cargo, S.FUENTES["bold"], S.TAMAÑOS["cuerpo"])
        period_w = c.stringWidth(periodo, S.FUENTES["regular"], S.TAMAÑOS["small"])

        # Wrap cargo if too wide
        if cargo_w + period_w + 20 > max_w:
            cargo_lines = _wrap_text(cargo, S.FUENTES["bold"], S.TAMAÑOS["cuerpo"], max_w - 10)
            for cl in cargo_lines:
                c.setFont(S.FUENTES["bold"], S.TAMAÑOS["cuerpo"])
                c.drawString(x_start, y, cl)
                y -= 13
            c.setFont(S.FUENTES["regular"], S.TAMAÑOS["small"])
            c.setFillColor(self.palette["texto_claro"])
        ...
        if ubic:
            y = self._draw_wrapped(y, ubic, x_start, max_w,
                                   font=S.FUENTES["regular"], size=S.TAMAÑOS["tiny"],
                                   color=self.palette["texto_claro"])

        y -= 1
        body_max = max_w - 10
        for resp in exp.get("responsabilidades", []):
            y = self._draw_wrapped(y, resp, x_start + 6, body_max, bullet=True,
                                   size=S.TAMAÑOS["small"])

        logros = exp.get("logros", [])
        if logros:
            y -= 2
            c.setFont(S.FUENTES["bold"], S.TAMAÑOS["small"])
            c.setFillColor(self.palette["verde"])
            c.drawString(x_start, y, "Logros:")
            y -= S.INTERLINEA["small"]
            for logro in logros:
                y = self._draw_wrapped(y, logro, x_start + 6, body_max, bullet=True,
                                       size=S.TAMAÑOS["tiny"])
    ...
    def _draw_skills(self, y, habilidades, x_start, x_end):
        ...
        for cat_key, cat_data in habilidades.items():
            ...
            lines = _wrap_text(line, S.FUENTES["regular"], S.TAMAÑOS["small"], max_w)
            c.setFont(S.FUENTES["regular"], S.TAMAÑOS["small"])
            c.setFillColor(self.palette["texto"])
            for ln in lines:
    ...
    def _build_single_col(self):
        ...
        y = self._draw_header(self.H - M - 20, x_start, x_end)
        ...
        if educacion:
            ...
            for edu in educacion:
                ...
                y = self._draw_wrapped(y, titulo, x_start, max_w,
                                       font=S.FUENTES["bold"], size=S.TAMAÑOS["cuerpo"])
    ...
    def _build_two_col(self):
        ...
        content_end = self.W - M - 4
        sidebar_x = M + 6
        sidebar_end = SIDEBAR_W - 6
        sidebar_max = sidebar_end - sidebar_x
        content_max = content_end - content_x

        y = self._draw_header(self.H - M - 20, M, self.W - M)
        ...
        if perfil:
            ...
            sy = self._draw_wrapped(sy, perfil, sidebar_x, sidebar_max,
                                    size=S.TAMAÑOS["small"])
            sy -= 6
        ...
        if habilidades:
            ...
            for cat_key, cat_data in habilidades.items():
                ...
                for item in items:
                    line = f"- {item}"
                    lines = _wrap_text(line, S.FUENTES["regular"], S.TAMAÑOS["small"], sidebar_max)
                    c.setFont(S.FUENTES["regular"], S.TAMAÑOS["small"])
                    c.setFillColor(self.palette["texto"])
                    for ln in lines:
        ...
        if educacion:
            ...
            for edu in educacion:
                ...
                c.setFont(S.FUENTES["bold"], S.TAMAÑOS["small"])
                c.setFillColor(self.palette["titulo"])
                titulo_lines = _wrap_text(titulo, S.FUENTES["bold"], S.TAMAÑOS["small"], sidebar_max)
                for ln in titulo_lines:
                    c.drawString(sidebar_x, sy, ln)
                    sy -= 11
                c.setFont(S.FUENTES["regular"], S.TAMAÑOS["tiny"])
                c.setFillColor(self.palette["texto_claro"])
                detail = f"{inst} ({edu.get('periodo', '')})"
                detail_lines = _wrap_text(detail, S.FUENTES["regular"], S.TAMAÑOS["tiny"], sidebar_max)
...
def generate_pdf(persona_id: str, derivado_id: str) -> Path:
    _register_fonts()
    data = build_cv_data(persona_id, derivado_id)
    output_dir = OUTPUT_DIR / persona_id
    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / f"{derivado_id}.pdf"
